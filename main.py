...
for url in links:
    ...
    for i in range(num_com-1):
        ...
        for i in html:
            ...
            if 'reviewPlus' in i and "reviewMinus" in i and "wDate" not in i:
                ...
                len_ot.append(len(i_dict["reviewPlus"] + i_dict["reviewMinus"]))
    ...
    if ',' in rating:
        ...
        dict_hotel[name] = float(rating)
    else:
        dict_hotel[name] = int(rating[0])
    for k in range(len(com)-1, -1, -1):
        if "rating" in com[k]:
            com[k] = (com[k]["reviewPlus"] + com[k]["reviewMinus"], com[k]["rating"])
        else:
            del com[k]
    dict_com[name] = com

    file.write(str(dict_hotel) + '\n')
    file.write(str(dict_com) + '\n')
